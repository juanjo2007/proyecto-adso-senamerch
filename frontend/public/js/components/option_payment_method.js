document.addEventListener('DOMContentLoaded', function () {
  ...
  if (methodContainer) {
    fetch('/frontend/public/views/components/option_payment_method.html')
      .then((response) => {
        if (!response.ok)
          throw new Error('Error al cargar option_payment_method.html');
      ...
      .then((data) => {
        ...
        const options = methodContainer.querySelectorAll(
        ...
        if (payButton) {
          payButton.addEventListener('click', () => {
            window.location.href = 'view_client_order.html';
          });
        }
      })
      .catch((error) =>
        console.error('Error cargando el componente Payment Method:', error)
      );
  } else {
